...
namespace IronSoul::Papyrus::Shardhearts
{
namespace
{
    ...
    enum class ShardheartEffect : std::int32_t
    {
        kTonalTemper = 1
    };
    ...
    struct ShardheartEnhanceOption
    {
        TonalToken token;
        std::string label;
    };

    struct ShardheartEnhanceSession
    {
        std::int32_t token{ 0 };
        ShardheartEffect effect{ ShardheartEffect::kTonalTemper };
        std::int32_t power{ 0 };
        std::int32_t cap{ 0 };
        std::vector<ShardheartEnhanceOption> options;
        std::unordered_map<std::int32_t, TonalToken> rowTokens;
    };

    std::unordered_map<std::int32_t, ShardheartEnhanceSession> g_enhanceSessions;
    ...
    ShardheartEnhanceOption BuildTonalEnhanceOption(
    ...
    std::optional<ShardheartEnhanceOption> BuildTonalEnhanceOptionFromInventoryEntry(
    ...
    std::vector<ShardheartEnhanceOption> BuildTonalEnhanceOptions(std::int32_t a_addLevels, std::int32_t a_maxTemperLevel)
    {
        std::vector<ShardheartEnhanceOption> options;
    ...
    static std::int32_t ShardheartBuildEnhanceSession(
        ...
        std::int32_t a_cap)
    {
        if (a_power <= 0 || a_cap <= 0) {
            SetLastResult(TonalResult::kInvalidRequest, "Invalid Shardheart enhancement request");
            return 0;
        }

        if (a_effectID != static_cast<std::int32_t>(ShardheartEffect::kTonalTemper)) {
            SetLastResult(TonalResult::kUnsupportedEffect, "Unsupported Shardheart enhancement effect");
            return 0;
        }

        ShardheartEnhanceSession session;
        session.token = NextEnhanceSessionToken();
        session.effect = ShardheartEffect::kTonalTemper;
        ...
        if (session.options.empty()) {
            ...
            logger::info("Iron Soul Shardheart: no eligible enhancement options effect={} power={} cap={}", a_effectID, a_power, a_cap);
            return 0;
        }
        ...
        logger::info(
            "Iron Soul Shardheart: built enhancement session token={} effect={} power={} cap={} options={}",
            sessionToken,
            a_effectID,
            a_power,
            a_cap,
    ...
    static std::int32_t ShardheartGetEnhanceSessionOptionCount(RE::StaticFunctionTag*, std::int32_t a_sessionToken)
    {
        auto sessionIt = g_enhanceSessions.find(a_sessionToken);
        if (sessionIt == g_enhanceSessions.end()) {
            SetLastResult(TonalResult::kInvalidToken, "Shardheart enhancement session is invalid or expired");
            return 0;
        }
    ...
    static std::string ShardheartGetEnhanceSessionOptionLabel(
        RE::StaticFunctionTag*,
        ...
        std::int32_t a_optionIndex)
    {
        ...
        if (a_optionIndex < 0 || static_cast<std::size_t>(a_optionIndex) >= options.size()) {
            SetLastResult(TonalResult::kInvalidOption, "Shardheart enhancement option is invalid");
            return "";
        }
    ...
    static std::string ShardheartRefreshEnhanceSessionInventoryRows(RE::StaticFunctionTag*, std::int32_t a_sessionToken)
    {
        auto sessionIt = g_enhanceSessions.find(a_sessionToken);
        if (sessionIt == g_enhanceSessions.end()) {
            SetLastResult(TonalResult::kInvalidToken, "Shardheart enhancement session is invalid or expired");
        ...
        if (session.rowTokens.empty()) {
            ...
            logger::info(
                "Iron Soul Shardheart: no eligible InventoryMenu rows for session={} effect={} power={} cap={}",
                a_sessionToken,
        ...
        SetLastResult(TonalResult::kOk, "OK");
        logger::info(
            "Iron Soul Shardheart: refreshed enhancement session rows session={} rows={}",
            a_sessionToken,
        ...
        const TonalToken& a_token)
    {
        ...
        if (!applied) {
            logger::warn(
                "Iron Soul Shardheart: apply failed session={} {}={} form={:08X}: {}",
                a_sessionToken,
                a_selectionKind,
                a_selectionIndex,
                a_token.baseFormID,
        ...
        logger::info(
            "Iron Soul Shardheart: applied enhancement session={} {}={} form={:08X} item='{}' mode={} power={} cap={} health={:.9f}->{:.9f} level=+{}->+{} result='{}'",
            a_sessionToken,
            a_selectionKind,
            a_selectionIndex,
            a_token.baseFormID,
    ...
    static bool ShardheartApplyEnhanceSessionOption(
        RE::StaticFunctionTag*,
        std::int32_t a_sessionToken,
        std::int32_t a_optionIndex)
    {
        ...
        ShardheartEnhanceSession session = std::move(sessionIt->second);
        ...
        if (session.effect != ShardheartEffect::kTonalTemper) {
            SetLastResult(TonalResult::kUnsupportedEffect, "Unsupported Shardheart enhancement effect");
            return false;
        }
    ...
    static bool ShardheartApplyEnhanceSessionInventoryRow(
        RE::StaticFunctionTag*,
        std::int32_t a_sessionToken,
        std::int32_t a_rowIndex)
    {
        ...
        if (rowIt == session.rowTokens.end()) {
            SetLastResult(TonalResult::kInvalidOption, "Shardheart enhancement inventory row is invalid");
            return false;
        }
    ...
    static void ShardheartReleaseEnhanceSession(RE::StaticFunctionTag*, std::int32_t a_sessionToken)
    {
        g_enhanceSessions.erase(a_sessionToken);
        SetLastResult(TonalResult::kOk, "OK");
    }

    static std::int32_t ShardheartGetEnhanceResult(RE::StaticFunctionTag*)
    {
        return static_cast<std::int32_t>(g_lastResult);
    }

    static std::string ShardheartGetEnhanceResultText(RE::StaticFunctionTag*)
    {
    ...
    {
        a_vm->RegisterFunction("ShardheartBuildEnhanceSession", IronSoul::Papyrus::kScriptName, ShardheartBuildEnhanceSession);
        a_vm->RegisterFunction("ShardheartGetEnhanceSessionOptionCount", IronSoul::Papyrus::kScriptName, ShardheartGetEnhanceSessionOptionCount);
        a_vm->RegisterFunction("ShardheartGetEnhanceSessionOptionLabel", IronSoul::Papyrus::kScriptName, ShardheartGetEnhanceSessionOptionLabel);
        a_vm->RegisterFunction("ShardheartRefreshEnhanceSessionInventoryRows", IronSoul::Papyrus::kScriptName, ShardheartRefreshEnhanceSessionInventoryRows);
        a_vm->RegisterFunction("ShardheartApplyEnhanceSessionOption", IronSoul::Papyrus::kScriptName, ShardheartApplyEnhanceSessionOption);
        a_vm->RegisterFunction("ShardheartApplyEnhanceSessionInventoryRow", IronSoul::Papyrus::kScriptName, ShardheartApplyEnhanceSessionInventoryRow);
        a_vm->RegisterFunction("ShardheartReleaseEnhanceSession", IronSoul::Papyrus::kScriptName, ShardheartReleaseEnhanceSession);
        a_vm->RegisterFunction("ShardheartGetEnhanceResult", IronSoul::Papyrus::kScriptName, ShardheartGetEnhanceResult);
        a_vm->RegisterFunction("ShardheartGetEnhanceResultText", IronSoul::Papyrus::kScriptName, ShardheartGetEnhanceResultText);
    }
}
